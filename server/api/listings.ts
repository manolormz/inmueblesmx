...
listings.get("/search", async (req, res) => {
  try {
    ...
    // If no DB configured, return simulated results
    if (!process.env.DATABASE_URL) {
      const base = { lat: 19.4326, lng: -99.1332 };
      const total = 240;
      const start = (page - 1) * pageSize;
      const results = Array.from({ length: pageSize }).map((_, i) => {
        const n = start + i + 1;
        return {
          listing_id: `mock-${n}`,
          operation: (req.query.operation as string) || "Sale",
          price: (req.query.operation === "Rent" ? 8000 : 1200000) + n * 1234,
          currency: "MXN",
          status: "published",
          property_id: `prop-${n}`,
          property_slug: `mock-${n}`,
          title: `Propiedad mock ${n}`,
          property_type: (req.query.type as string) || "Casa",
          bedrooms: 2 + (n % 3),
          bathrooms: 1 + (n % 2),
          built_area_m2: 60 + (n % 80),
          state_slug: "cdmx",
          municipality_slug: "benito-juarez",
      ...
      return res.json({ page, pageSize, total, results, simulated: true });
    }
    ...
    res.status(500).json({ error: "DB unavailable", message: String(err?.message || err) });
  }
});
...
listings.get("/:slugOrId", async (req, res) => {
  try {
    if (!process.env.DATABASE_URL) {
      return res.status(404).json({ error: "Listing not found (dev)" });
    }

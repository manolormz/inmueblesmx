...
comms.post('/leads', async (req, res) => {
  const { listing_id, name, email, phone_e164, message, buyer_id } = req.body || {};
  ...
  const { rows } = await query(
    `INSERT INTO lead (listing_id, buyer_id, name, email, phone_e164, message, source)
     ...
     RETURNING *;`,
    [listing_id, buyer_id || null, name || null, email || null, phone_e164 || null, message || null]
  );
...
comms.post('/conversations', async (req, res) => {
  ...
  if (!listing_id || !buyer_id || !agent_id) {
    return res.status(400).json({ error: 'listing_id, buyer_id, agent_id required' });
  }
...
comms.post('/conversations/:id/messages', async (req, res) => {
  ...
  await query(`UPDATE conversation SET last_message_at = now() WHERE id = $1;`, [req.params.id]);
  res.status(201).json(rows[0]);
});

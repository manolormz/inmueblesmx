...
export const listSeededProperties: RequestHandler = async (req, res) => {
  try {
    ...
    const pageSize = Math.max(
      1,
      Math.min(100, parseInt(String(req.query.pageSize || "20"), 10) || 20),
    );
...
export const unpublishProperty: RequestHandler = async (req, res) => {
  try {
      ...
      if (isProd) {
        return res
          ...
          .json({
            ok: false,
            message:
              "Falta configuraci��n del servidor (BUILDER_PRIVATE_API_KEY)",
          });
      } else {
        return res
          .status(200)
          .json({
            ...
            devHint: true,
            message:
              "Falta BUILDER_PRIVATE_API_KEY; ejecutando en modo demo (no se guardó en CMS).",
          });
      }
    }
    ...
    const put = await fetch(
      `https://builder.io/api/v3/content/${id}?apiKey=${privateKey}`,
      {
        method: "PUT",
      ...
      if (isProd) {
        return res
          ...
          .json({
            ok: false,
            message:
              "Falta configuración del servidor (BUILDER_PRIVATE_API_KEY)",
          });
      } else {
        return res
          .status(200)
...
export const updateProperty: RequestHandler = async (req, res) => {
  try {
    ...
    if (!slug)
      return res.status(400).json({ ok: false, message: "Falta slug" });
    ...
    if (
      ...
      const base = (await import("../../shared/formatters")).slugifyEs(
        input.title.trim(),
      );
      ...
      const exists =
        Array.isArray(cj?.results) &&
        cj.results.some((it: any) => it?.id && it.id !== id);
    ...
    try {
      ...
      return res
        .status(400)
        .json({ ok: false, message: e?.message || "Datos inválidos" });
    }
    ...
    if (!put.ok) {
      ...
      return res
        .status(500)
        .json({ ok: false, message: "Builder API error", detail });
    }

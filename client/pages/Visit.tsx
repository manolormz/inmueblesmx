...
export default function VisitPage() {
  ...
  const {
    ...
    reset,
  } = useZodForm(VisitInput, { listingId });
  const { toast } = useToast();
  const nav = useNavigate();

  useEffect(() => {
    if (listingId) reset({ listingId });
  }, [listingId, reset]);
      ...
      <div className="max-w-lg mx-auto bg-white border rounded-2xl p-4 space-y-3">
        ...
        {!listingId && (
          <p className="text-sm text-yellow-700 bg-yellow-50 border border-yellow-200 rounded-md p-2">
            No se detectó listingId en la URL. Puedes pegarlo manualmente.
          </p>
        )}
        <form
        ...
        >
          {!listingId && (
            <FormField
              ...
              required
              error={errors.listingId?.message as any}
            >
          ...
          <FormField
            ...
            error={errors.datetime?.message as any}
          >
          ...
          />
          <ActionBar
            primaryText={isSubmitting ? "Solicitando…" : "Solicitar"}
            busy={isSubmitting}
          />
        </form>
      </div>
